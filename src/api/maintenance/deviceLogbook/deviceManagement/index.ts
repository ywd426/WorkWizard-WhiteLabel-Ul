...
export function list(query: DeviceQuery): AxiosPromise<any> {
  return request({
    url: '/device/management/formInfo',
    method: 'get',
    params: query
  });
}
...
export function logNameList(category: string, equipmentId: string): AxiosPromise<any> {
  const query = {
    category,
    equipmentId
  };
  return request({
    url: '/maintenance/dict/type/logNameList',
    method: 'get',
    params: query
  });
}
export function logTagList(): AxiosPromise<any> {
  return request({
    url: '/maintenance/dict/type/logTagList',
    method: 'get'
  });
}
