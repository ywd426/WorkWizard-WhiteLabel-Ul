 ...
 * @param newPassword 新密码
 */
export const updatePwdNoOldPwd = (email: string, newPassword: string) => {
  const data = {
    email,
    newPassword
  };
  return request({
    url: '/system/user/profile/updatePwdNoOldPwd',
    method: 'put',
    headers: {
      isEncrypt: true,
      repeatSubmit: false
...
export function remindUserList(): AxiosPromise<any> {
  return request({
