<template>
  ...
  >
    <template #header-actions>
      <ActionMenu @create="handleCreateTask" @upload="handleUploadExcel" />
    </template>
    <template #action-buttons="{ task, localtionId }">
      ...
      <div class="flex flex-row">
        ...
        <el-button circle size="middle" @click="assignTask(task)">
          <svg-icon icon-class="assign-card" class="svg-default" />
        </el-button>
  ...
  <delete-dialog v-model:visible="showDeleteDialog" @delete="handleDelete" :name="deleteTaskId" title="Delete Work Order" type="work order" />
  ...
  <edit-task-template
    ...
    :header="editHeader"
    @save="handleSave"
    prompt="Are you sure you want to delete this work order?"
...
import { getActiveTasks, getZoneActives, getWorkOrderDetail, deleteWorkOrder } from '@/api/clean/task';
...
const handleSave = (saveData) => {
  console.log('Saving task...', saveData);
...
const getActiveTasksData = async (task) => {
  ...
  const res = await getZoneActives({ ...params, zoneId: task.id });
...
const fetchData = async () => {
  ...
  const res = await getActiveTasks({ ...params, ...pageQuery });
...
const viewTask = async (task) => {
  type.value = 'view';
  header.value = 'View Work Order'; // Update the header he
  editHeader.value = `View Active Work Order`;
...
const editTask = async (task) => {
  ...
  curTask.value = task;
  editHeader.value = `Edit Active Work Order`;
  const res = await getWorkOrderDetail(task.id);
  curData.value = res.data;
  showEditDialog.value = true;
...
const editTaskSubmit = (task) => {
  console.log('edit task submit:', task);
};

const assignTask = (task) => {
  task.editable = true; // 标记为可编辑状态
...
const handleDelete = async (name) => {
  console.log(`delete task: ${name}`, locationId);
