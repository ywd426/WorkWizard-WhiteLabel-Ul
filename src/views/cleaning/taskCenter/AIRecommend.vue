<template>
  <TaskList
    ref="taskListRef"
    header="AI-Recommended Work Orders"
    :showTabs="false"
    :showPending="false"
  ...
  >
    <template #action-buttons="{ task, localtionId }">
      ...
      <div class="flex flex-row">
        <el-button id="add-icon" circle size="middle" @click="moveTask(task)">
          <svg-icon icon-class="move-card" class="svg-default" />
        </el-button>

        <el-button circle size="middle" @click="ignoreTask(task, localtionId)">
          <svg-icon icon-class="ignore-alert" class="svg-default" icon="ignore" color="white"></svg-icon>
        </el-button>
      </div>
    </template>
  </TaskList>
...
import { getAIRecommends, getZoneRecommends, getWorkOrderDetail, deleteWorkOrder } from '@/api/clean/task';
...
const deleteHeader = ref('Delete Work Order');
// 任务数据
const taskData = ref([]);
const total = ref(0);
...
const fetchData = async () => {
  ...
  const res = await getAIRecommends({ ...params, ...pageQuery });
  ...
  total.value = res.total || filteredData.length;
};
...
const viewTask = async (task) => {
  header.value = 'View Work Order';
  type.value = 'view';
  const res = await getWorkOrderDetail(task.id);
  editHeader.value = `View AI-Recommended Work Orders`;
...
const moveTask = (task) => {
  promptInfo.value = "Are you sure you want to move it to active work order list?";
  isPrimary.value = true;
  isDanger.value = false;
  header.value = 'Move Work Order';

  showDeleteDialog.value = true;
  console.log('move task:', task);
};

const handleDelete = async (name, isDelete) => {
  console.log(`delete task: ${name}`,isDelete, locationId);
  const res = await deleteWorkOrder(name);
...
const ignoreTask = (task, localtionId) => {
  deleteTaskId.value = task.id || task.name;
  promptInfo.value = "Are you sure you want to ignore this work order?";
  isPrimary.value = false;
  isDanger.value = true;
  header.value = 'Delete Work Order';
...
const getActiveTasksData = async (task) => {
  const params = cleanStore.getPageFilterParams(props.pageId);
  const res = await getZoneRecommends({ ...params, zoneId: task.id });
