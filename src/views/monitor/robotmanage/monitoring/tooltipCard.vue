<template>
  <el-row
    id="default-board-card"
    v-loading="cardLoading"
    v-if="showCard"
    :gutter="singleMode ? 24 : 20"
    :style="{
      width: '100%',
        ...
        <el-col v-for="detail in getDetailList(item.name)" :key="detail.sensorId" :span="24">
          <el-row class="box-card" :class="detail.status == '3' ? 'disabled-overlay' : ''">
            ...
            <el-col :span="24" class="item-title">
              <!-- <span class="font-item-b12 line" v-tooltip="detail.sensorName">
                {{ detail.sensorName }}
              </span> -->
              <el-tooltip :visible="visible" placement="top"  :content="detail.sensorName">
                <span class="font-item-b12 line" @mouseenter="visible = true" @mouseleave="visible = false">
                  {{ detail.sensorName }}
                </span>
              </el-tooltip>
            ...
            <el-row style="padding: 8px 0; width: 100%">
              ...
              <el-col v-if="detail.location" :span="24" style="display: flex; align-items: center; justify-content: space-between">
                <el-tooltip placement="top" :content="detail.location">
                  <span class="font-item-b69 line">{{ detail.location }}</span>
                </el-tooltip>
...
const handleRowClick = (robotId: string) => {
  emits('viewDetail', data.value);
};
const initData = (nums: number, list: SensorForm[], detailListData?: [], node?: {}) => {
