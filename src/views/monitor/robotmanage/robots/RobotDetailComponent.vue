<template>
  <div ref="p2" class="p-2" v-loading="loading">
    <div class="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-6 gap-4">
      ...
      <div>
        <div class="robot-show-col">
          ...
          <span class="total-area">{{ convertWithUnit({ value: curDetail.totalArea, type: 'area' }) }}</span>
        </div>
      </div>
      <div v-if="isToday">
        <div class="robot-show-col">
      ...
      <el-col :span="24" class="robot-detail">
        ...
        <el-row class="detail-body">
          <el-col :span="24" :model="detailForm" class="detail-info">
            ...
            <el-form-item label="Name" label-position="top" class="detail-item" style="font-weight: 400">
              <div class="robot-name" style="display: flex; align-items: center; gap: 8px">
                <div class="w-full flex gap-2 items-center" v-if="isEditingName">
                  <el-input
                  ...
                  <svg-icon
                    icon-class="saveInfo"
                    @click="saveRobotName"
                    style="width: 16px; height: 16px; cursor: pointer; color: #11191e"
                    title="Save name"
                  />
                </div>
                <div v-else class="w-full flex gap-2 items-center">
                  <el-text style="font-size: 16px; font-weight: 700; flex: 1">
                    {{ curDetail.robotName }}
                  </el-text>
                  <el-icon @click="startEditName" style="cursor: pointer" title="Edit name"><Edit /></el-icon>
                </div>
              </div>
            </el-form-item>
...
import { saveSNName, getRobotDetail, getCustomFields, saveCustomFields, removeCustomFieldsById, removeRobotCustomFields } from '@/api/robot';
...
const getSensorCustomFields = async () => {
  await getCustomFields(props.robotData.robotSn, 'robot').then((res) => {
    editList.value = res.data;
  });
};
const addCustomFields = async () => {
  detailCustomForm.value = { relateId: '', type: 'robot', details: [] };
  detailCustomForm.value.relateId = props.robotData.robotSn;
  editList.value.forEach((item) => {
...
const deleteCustomFields = async () => {
  await removeRobotCustomFields(props.robotData.robotSn, 'robot').then((res) => {
    proxy?.$modal.msgSuccess('Operation Successful');
  });
};
...
const saveRobotName = async () => {
  ...
  try {
    ...
    // await updateRobotName(props.robotData.robotSn, editingRobotName.value);

    // 暂时直接更新本地数据，实际项目中应该调用API
    const res = await saveSNName(props.robotData.robotSn, editingRobotName.value);
    if (res.code === 200) {
...
const isToday = computed(() => {
  return props.robotData.isToday;
});
onMounted(async () => {
  const data = await getRobotDetail(props.robotData.robotSn);
  curDetail.value = data.data || {};
  curDetail.value.totalTasks = props.robotData.totalTasks || curDetail.value.totalTasks;
  curDetail.value.totalRunningHours = props.robotData.totalRunningHours || curDetail.value.totalRunningHours;
  curDetail.value.totalArea = props.robotData.totalArea || curDetail.value.totalArea;
